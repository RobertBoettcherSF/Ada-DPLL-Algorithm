...
begin
   ...
   begin
      Check ("1.1 Make_Pos produces positive sign", L_Pos.Sign = Positive and L_Pos.Var = 3);
      Check ("1.2 Make_Neg produces negative sign", L_Neg.Sign = Negative and L_Neg.Var = 3);
      Check ("1.3 Double negation identity", Negate (Negate (L_Pos)) = L_Pos and Negate (Negate (L_Neg)) = L_Neg);
   end;
   ...
   declare
      Empty_F : constant Formula := Formula'(Num_Clauses => 0, Num_Vars => 2, Clauses => (others => Clause'(Length => 0, Lits => (others => Make_Pos (1)))));
   ...
   begin
      ...
      begin
         ...
         Check ("4.3 Unit propagation alone detects contradiction", Unit_Propagation_Only (F).Status = Unsatisfiable);
      end;
   end;
   ...
   declare
      --  (A or B) AND (A or ~B): A is strictly positive (pure), B occurs positive & negative
   ...
   declare
      --  Clause setup where variable 3 appears in multiple clauses
      F : Formula (Num_Clauses => 3, Num_Vars => 3);
   begin
   ...
   declare
      --  Variable 2 appears in shortest clauses
      F : Formula (Num_Clauses => 3, Num_Vars => 3);
   begin
   ...
   declare
      --  3 pigeons (1,2,3), 2 holes (H1, H2). P_i_j = Pigeon i in Hole j.
      --  Var indices: P11=1, P12=2, P21=3, P22=4, P31=5, P32=6
      --  Every pigeon must be in a hole:
      --  (P11 or P12), (P21 or P22), (P31 or P32) -> 3 clauses
      --  No hole has two pigeons:
      --  (~P11 or ~P21), (~P11 or ~P31), (~P21 or ~P31)
      --  (~P12 or ~P22), (~P12 or ~P32), (~P22 or ~P32) -> 6 clauses
      --  Total 9 clauses, 6 variables.
   ...
   declare
      --  Clause references Variable 4, but Num_Vars is declared as 2
